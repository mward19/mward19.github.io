...
#show: template.with(title: "3DEO Illumination Spot Modeling")

#article-heading[Internship with 3DEO---Illumination Spot Modeling][September 13, 2026]
...
#picture(image("imgs/main-37.png"))
...
#picture(html.video(
  controls: true,
  style: "width: 90%",
  src: "/assets/2025/video/hitmap_moving_average_window_100_slow.mp4",
)[
...
#picture(image("imgs/main-42.png"))
...
Since that didn't work, I also tried fitting a truncated 2-dimensional Gaussian to the data with Bayesian inference (using PyMC). It was a lot of fun, but found that one of the tools I needed #link("https://discourse.pymc.io/t/truncated-multivariate-normal-likelihood/17032")[wasn't yet implemented].

#html.hr()

As I dug into this problem, I found that some Princeton astronomers had faced the same challenge while fitting Gaussian Mixture Models to images of galaxies. They made some extensions to Expectation Maximization (the standard algorithm for fitting GMMs) in order to handle incomplete data like mine (#link("https://arxiv.org/abs/1611.05806")[paper]). I applied their code, #link("https://github.com/pmelchior/pygmmis")[a Python library called pygmmis], to my problem and it fit very nicely.

#picture(html.video(
  controls: true,
  style: "width: 90%",
  src: "/assets/2025/video/spot_movement_window_100_gaussians_1_new_gmmis_1_in_1_slow.mp4",
)[
  Your browser does not support the video tag.
])
...
- Prototype a simple pointwise reflectivity estimate. This might look something like $ "reflectivity estimate" prop "observed reflection intensity"/"illumination intensity" $ where $"illumination intensity"$ is inferred from the spot illumination model, and $"observed reflection intensity"$ comes from the local density of the point cloud.
