...
#show: template.with(title: "A cheaper alternative to tablets, and the mouse to rule them all")
...
The trickiest part about all this is how to set it up. Naturally with unstandardized hardware like a screenless tablet there are lots of driver issues and subtleties to keep in mind. In addition, I use Linux (Debian 12 with GNOME and KDE), so things are often a little less intuitive (but more fun). Huion (the manufacturer) makes a good driver, and I also like using #link("https://opentabletdriver.net/")[OpenTabletDriver], an open-source alternative. Things get a little dicey with a multi-monitor setup but I think I've got it figured out now.

I take notes in #link("https://xournalpp.github.io/")[Xournal++]. I love it. It's everything I needed in notetaking software and more. My tablet has eight buttons on the left, which I've mapped to the following in Xournal++: undo, delete, pencil, zoom in, zoom out, new page, the lasso selection tool, and the hand tool. I also have an on-screen icon in my GNOME panel to take screenshots (#link("https://extensions.gnome.org/extension/1112/screenshot-tool/")[Screenshot Tool]), which is especially useful when I want to paste snippets from textbooks (all of which I access digitally) into my homework. As a result, my workflow is very fast! 

I deliberately chose to map the buttons to shortcuts for which I wouldn't want to move my pen. For example, if I'm writing a word and I mess up, I can undo my mistake without having to change my pen position, so I can rewrite it immediately. I can select and move a group of objects with the lasso without having to move away. I have come to realize that it is much faster than a traditional tablet because of these shortcut keys. Maybe an iPad would be better with some kind of shortcut keypad... #link("https://a.co/d/9EC01zY")[looks like that exists too].
...
#picture(image("imgs/tablet-on-keyboard.jpg", alt: "Tablet on keyboard"))

When I *do* have room to spread out, I take advantage of the space.

#picture(image("imgs/devices-full-setup.jpg", alt: "Full setup"))
...
#picture(image("imgs/hand-on-trackball.jpg", alt: "My hand on the trackball"))
