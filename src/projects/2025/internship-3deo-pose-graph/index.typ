...
#site-page("projects/2025/internship-3deo-pose-graph/index.html", tab-title: "3DEO Pose Graph Optimization")[
  ...
  #large-img("imgs/registrations.png")
  ...
  #large-img("imgs/main-19.png")

  We can set this problem up as a _pose graph_, a kind of mathematical #link("https://en.wikipedia.org/wiki/Graph_(discrete_mathematics)")[graph]. In it, each node ($X_1, ..., X_n$) represents the position of a scan relative to where it "belongs". Each edge (#inline-frame[$macron(D)_(i j), i, j in {1, ..., n}$], transformation from scan $j$ to scan $i$) represents a pairwise alignment (also called a pairwise _registration_) derived from an algorithm like ICP. Some of the edges #inline-frame[$macron(D)_(i j)$] are known, and each $X_i, i in {1, ..., n}$ is unknown. Now, one would hope that taking scan 3 and aligning with scan 1 would be equivalent to taking scan 3, aligning it with scan 2, and aligning that to scan 1, but in practice this is never the case since no pairwise registration algorithm is perfect. That is, #inline-frame[$macron(D)_13 != macron(D)_12 macron(D)_23$].

  #html.hr()

  To determine the optimal pose of each scan (the value of each $X_i$), we can imagine that each pairwise registration is a spring in the pose graph. Since poses are multidimensional, the "elasticity" of each spring is a covariance matrix $C_(i j)$, and the energy in each spring given a set of poses $X_1, ..., X_n$ is the squared Mahalanobis distance #centered-frame[$ (macron(D)_(i j) - (X_i - X_j))^T C_(i j)^(-1) (macron(D)_(i j) - (X_i - X_j)). $] The optimal poses minimize the sum of the energy in the springs. That is, we seek

  #centered-frame[$
    limits("argmin")_(X_1, ..., X_n) (macron(D)_(i j) - (X_i - X_j))^T C_(i j)^(-1) (macron(D)_(i j) - (X_i - X_j)).
  $]

  #large-img("imgs/main-21.png")
  ...
  #large-img("imgs/main-29.png")
  #large-img("imgs/main-30.png")

  #large-img("imgs/main-31.png")
  #large-img("imgs/main-32.png")
