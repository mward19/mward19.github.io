...
#site-page("projects/2024/predicting-life-expectancy/index.html", tab-title: "Predicting future life expectancy in countries using present data")[
  ...
  This project is a #link("documents/Life_Expectancy_Paper.pdf")[paper] with associated #link("documents/Life_Expectancy_Paper_code.pdf")[code].
  ...
  I usually manage my code with Git and GitHub, but this time I opted for a Jupyter notebook in Google Colab to avoid training the models on my system. #link("documents/Life_Expectancy_Paper_code.pdf")[View the code here]. It proved challenging to export the large notebook to other formats (.pdf, .html). Troubleshooting would've been easier had I coded in a more robust way, by saving trained models and such. The time I spent wrestling with the notebook export reminded me to be more careful next time, in spite of any looming deadlines.

  The #link("documents/Life_Expectancy_Paper.pdf")[original paper] is a .pdf document, but for a better reading experience, I'll copy it as best I can below.
  ...
  *Abstract.* Predicting life expectancy is crucial for understanding global health trends and guiding policy decisions. We aim to predict the life expectancy of a country five years into the future using socio-economic and health-related features. After engineering features to normalize trends and focus on deviations from global averages, we employ several machine learning models, including Random Forest and Gradient Boosting, to make predictions. Initial results demonstrate modest predictive performance when evaluated on unseen countries, with features like income composition and vaccination rates contributing significantly. However, cluster-based evaluation reveals that the models struggle to generalize across diverse regions, highlighting challenges in capturing global heterogeneity.
  ...
  Using the #link("https://www.kaggle.com/datasets/kumarajarshi/life-expectancy-who/data")[WHO Life Expectancy dataset], _how well can we predict the life expectancy of a given country in 5 years from now?_

  Previous research done with similar datasets used a variety of techniques. #link("https://doi.org/10.1007/s42452-023-05404-w")[Lipesa, et al.] used extreme gradient boosting (XGBoost) and pointed to thinness, schooling, infant deaths and BMI as leading factors in Life Expectancy. #link("https://ieeexplore.ieee.org/document/10441837")[Gill et al.] used linear regression to find a correlation between adult mortality and GDP per capita.

  The #link("https://www.kaggle.com/datasets/kumarajarshi/life-expectancy-who/data")[Life Expectancy Dataset] contains data from countries over a fifteen year period. Below we outline a few.

  #table(
    columns: 2,
    table.header([Feature], [Explanation]),

    [_Life Expectancy_], [Average national life expectancy (years)],
    [_Alcohol_], [Alcohol consumed per capita (liters)],
    [_Percentage expenditure_], [Government expenditure on health (% of GDP)],
    [_Polio_], [Percent population vaccinated against polio],
  ...
  We also noticed that there were problems with the "GDP" data column. Much of it was missing or inaccurate. For this reason, we chose to take the GDP from #link("https://www.kaggle.com/ds/3065197")[another dataset], which we verified had accurate and complete numbers by comparing a random subset of the numbers to those found by the #link("https://databank.worldbank.org/indicator/NY.GDP.PCAP.CD/1ff4a498/Popular-Indicators")[World Bank].

  We imputed the rest of the missing data using an imputer built with the k-Nearest Neighbors algorithm (#link("https://scikit-learn.org/stable/modules/generated/sklearn.impute.KNNImputer.html")[KNNImputer from scikit-learn]).
  ...
  #figure(
    large-img("imgs/le_worldwide_mean.jpg", width: 60%),
  ...
  #figure(
    large-img("imgs/le_unstable.png", width: 70%),
    alt: "Life expectancy trends in various countries compared with the mean",
    caption: [Life expectancy is not always stable.]
  )
  ...
  #figure(
    large-img("imgs/le_alcohol.png", width: 90%),
    alt: "Alcohol consumption alone is not sufficient to predict life expectancy",
    caption: [Prediction of life expectancy is not trivial.]
  )
  ...
  #figure(
    [
      #large-img(
        "imgs/le_pred_Austria.jpg",
        alt: "Predicting deviation of life expectancy from worldwide mean (Austria)",
        width: 52%
      )
      #large-img(
        "imgs/le_pred_Rwanda.jpg",
        alt: "Predicting deviation of life expectancy from worldwide mean (Rwanda)",
        width: 52%
      )
    ],
    alt: "Alcohol consumption alone is not sufficient to predict life expectancy",
    caption: [The model is able to make some meaningful predictions (top), but sometimes fails (bottom). Since the model uses data from a given year to predict the life expectancy five years from then, the predicted line begins in 2005 and continues to 2020. The outputs of the model give the _change_ in life expectancy over 5 years, so adding those to the current life expectancy gives us the orange line as shown.]
  )
  ...
  #figure(
    large-img("imgs/le_clusters.png", width: 70%),
    alt: "Country clusters",
    caption: [The clusters visualized using the first two principal components. See Appendix B.]
  )
